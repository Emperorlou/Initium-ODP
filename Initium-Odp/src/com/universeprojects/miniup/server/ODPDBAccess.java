...
public class ODPDBAccess
{
	...
	private List<CachedEntity> getTerritoryCharacters(TerritoryCharacterFilter filter, CachedEntity startLocation, boolean single)
	{
		if (startLocation==null) return null;
		Key territoryKey = (Key)startLocation.getProperty("territoryKey");
		if (territoryKey==null) return null;
		CachedEntity territory = getEntity(territoryKey);
		if (territory==null) return null;
		CachedDatastoreService db = getDB();
		Filter f0 = new FilterPredicate("territoryKey", FilterOperator.EQUAL, territoryKey);
		List<CachedEntity> territoryLocations = db.fetchAsList("Location", f0, 10000);
		List<List<CachedEntity>> tlcList = new ArrayList<List<CachedEntity>>();
		getTerritoryLocationCharacters(tlcList, db, filter, startLocation, 0, territory, territoryLocations, single);
	...
	public boolean getTerritoryLocationCharacters(List<List<CachedEntity>> tlcList, CachedDatastoreService db, TerritoryCharacterFilter filter, CachedEntity location, int distance, CachedEntity territory, List<CachedEntity> territoryLocations, boolean single)
	{
		...
		List<CachedEntity> characters = db.fetchAsList("Character", f0, 10000);
		...
		{
			// If only 1 character is required and we have one, exit early
			if (getTerritoryLocationCharacters(tlcList, db, filter, l, distance+1, territory, territoryLocations, single) && single)
				return true;
		}
