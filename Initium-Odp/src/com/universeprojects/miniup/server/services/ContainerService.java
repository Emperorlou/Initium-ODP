...
public class ContainerService extends Service {
	...
	public void doUse(CachedDatastoreService ds, final CachedEntity container,
			long durabilityLoss) throws UserErrorMessage {
		long durability = (long) container.getProperty("durability");

		if (durability > 0) {
			container.setProperty("durability", durability - durabilityLoss);
		} else { // items inside are moved to the container's container, and
					// container is destroyed {
			CachedEntity parentContainer = db.getEntity((Key) container
					.getProperty("containerKey"));
			List<CachedEntity> items = db.getFilteredList("Item",
					"containerKey", FilterOperator.EQUAL,
					parentContainer.getKey());
			...
			throw new UserErrorMessage(
					"The container has been destroyed due to durability loss, and any items inside have fallen out of it.");
		}
	}

	/**
	 * This is for stuff that allows access to a given container (location,
	 * item, or character). Returns false if access should not be allowed.
	 * 
	 * @param character
	 * @param container
	 * @return
	 */
	public boolean checkContainerAccessAllowed(CachedEntity character,
			CachedEntity container) {
		// If the container is ourselves, it's ok
		if (container.getKind().equals("Character")
				&& GameUtils.equals(character.getKey(), container.getKey()))
			return true;

		// If the container is our location, it's ok
		if (container.getKind().equals("Location")
				&& GameUtils.equals(character.getProperty("locationKey"),
						container.getKey()))
			return true;

		// If the container is an item in our inventory, it's ok
		if (container.getKind().equals("Item")
				&& GameUtils.equals(character.getKey(),
						container.getProperty("containerKey")))
			return true;
	...
	/**
	 * 
	 * Retrieves the display name to use for a specific container. Will return
	 * the container's label unless the label is null or empty. If label is null
	 * or empty, will return the item name.
	 * 
	 * @param container
	 * @return
	 */
