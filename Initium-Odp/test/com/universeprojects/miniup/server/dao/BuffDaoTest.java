...
public class BuffDaoTest {

	private final LocalServiceTestHelper helper = new LocalServiceTestHelper(new LocalDatastoreServiceTestConfig());

	private BuffDao testObj;

	@Before
	public void before() {
		helper.setUp();
		CachedDatastoreService.disableRemoteAPI();
		testObj = new BuffDao(new ODPDBAccess(new HttpServletRequestImpl()).getDB());
	}
	...
	public void saveAndGet() { // This works because of the caching
		Buff expectedBuff = new Buff();

		testObj.save(expectedBuff);
		Buff actualBuff = testObj.get(expectedBuff.getCachedEntity().getKey());
