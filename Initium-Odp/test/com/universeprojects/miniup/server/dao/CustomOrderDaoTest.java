...
public class CustomOrderDaoTest {
	...
	private CustomOrderDao testObj;

	@Before
	public void before() {
		helper.setUp();
		CachedDatastoreService.disableRemoteAPI();
		testObj = new CustomOrderDao(new ODPDBAccess(new HttpServletRequestImpl()).getDB());
	}
	...
	public void saveAndGet() { // This works because of the caching
		...
		testObj.save(expectedCustomOrder);
		CustomOrder actualCustomOrder = testObj.get(expectedCustomOrder.getCachedEntity().getKey());

		assertEquals(expectedCustomOrder, actualCustomOrder);
	}
}
